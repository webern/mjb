    ...
    <div className="card-footer">
      ...
      <span className="mod-options">
        <i
          ...
          onClick={reduce(_event => DeleteComment(comment.id))}
        />
      </span>
    </div>
  </div>;
...
let followUser = (isFollowing, event) =>
  isFollowing ?
    UnFollowUser(
      ReactDOMRe.domElementToObj(ReactEvent.Mouse.target(event))##value,
    ) :
    FollowUser(
      ReactDOMRe.domElementToObj(ReactEventRe.Mouse.target(event))##value,
    );
